...
NS_EOSC_LIFECYCLESTATUS = (
    "https://vocabs.sshopencloud.eu/vocabularies/eosc-life-cycle-status/"
)
NS_EOSC_TRL = (
    "https://vocabs.sshopencloud.eu/vocabularies/eosc-technology-readiness-level/"
)
...
class MarketPlaceAPI:
    def __init__(
        ...
        if username and password:
            ...
            self.bearer = response.headers[
                "Authorization"
            ]  # this includes the bearer prefix
            if self.debug:
    ...
    def get_source(self, url: str) -> dict:
        ...
        response = requests.get(
            f"{self.baseurl}/api/sources", params={"q": url}, headers=self.headers()
        )
    ...
    def add_source(self, label: str, url: str, urltemplate: str) -> dict:
        ...
        response = requests.post(
            f"{self.baseurl}/api/sources", headers=self.headers(), json=payload
        )
    ...
    def get_actor(self, name: str) -> dict:
        ...
        response = requests.get(
            f"{self.baseurl}/api/actor-search",
            params={"q": name.strip()},
        ...
        return response.json()["actors"][
            0
        ]  # returns the first match! (may not be what you want if there are multiple)
    ...
    def add_actor(
        ...
        response = requests.post(
            f"{self.baseurl}/api/actors", headers=self.headers(), json=payload
        )
        self.validate_response(response, payload, "add_actor")
        return response.json()

    def get_or_add_actor(
        self,
        name: str,
        website: Optional[str],
        email: Optional[str],
    ...
    def get_keyword(self, keyword: str) -> dict:
        ...
        response = requests.get(
            f"{self.baseurl}/api/concept-search",
            params={"q": label.strip(), "types": "keyword"},
            headers=self.headers(),
        )
        ...
        for concept in response.json()["concepts"]:
            if (
                concept["code"] == code
                or concept["label"].strip().lower() == label.lower()
            ):
                # require exact match
                return concept
        raise KeyError
    ...
    def add_keyword(self, keyword: str) -> Optional[dict]:
        ...
        response = requests.post(
            f"{self.baseurl}/api/vocabularies/sshoc-keyword/concepts",
            headers=self.headers(),
            json=payload,
        )
    ...
    def get_license(self, code: str) -> dict:
        ...
        response = requests.get(
            f"{self.baseurl}/api/concept-search",
            params={"q": code.strip(), "types": "license"},
            headers=self.headers(),
        )
    ...
    def add_thumbnail(self, url: str) -> dict:
        ...
        response = requests.post(
            f"{self.baseurl}/api/media/upload/import",
            json={"sourceUrl": url},
            headers=self.headers(),
        )
    ...
    def add_tool(self, data: dict):
        ...
        response = requests.post(
            f"{self.baseurl}/api/tools-services", headers=self.headers(), json=data
        )
    ...
    def update_tool(self, persistent_id: str, data: dict):
        ...
        response = requests.patch(
            f"{self.baseurl}/api/tools-services/" + persistent_id,
            headers=self.headers(),
            json=data,
        )
    ...
    def validate_response(
        self, response: requests.Response, data: Union[dict, None], context: str
    ):
        """Internal helper method to validate responses and raise errors if needed"""
        if response.status_code < 200 or response.status_code >= 300:
            print(
                f"-------- ERROR {response.status_code} in {context} ---------",
                file=sys.stderr,
            )
            if data:
...
def normalize_keyword(keyword: str) -> tuple[str, str]:
    ...
    keyword = re.sub(
        r"[^\w ]+", "-", re.sub("_+", "-", keyword)
    )  # all punctuation becomes a hyphen
    code = keyword.lower().replace(
        " ", "-"
    )  # the ID code uses hyphens instead of spaces and is all lowercase
    label = keyword
    return (code, label)
    ...
    for i, (_, _, o) in enumerate(l):
        if isinstance(o, Literal):
            ...
            if str(o).startswith("https://orcid.org/"):
                orcid = str(o)
            elif g.value(o, OWL.sameAs, None) and str(
                g.value(o, OWL.sameAs, None)
            ).startswith("https://orcid.org/"):
...
def main():
    parser = argparse.ArgumentParser(
        prog="codemeta2mp", description="Converts codemeta to SSHOC Open Marketplace"
    )
    parser.add_argument(
        "--baseurl",
        help="Marketplace API base url",
        type=str,
        default="https://marketplace-api.sshopencloud.eu",
    )
    ...
    parser.add_argument(
        "--sourcelabel", help="Source label", type=str, default="CLARIAH-NL Tools"
    )
    parser.add_argument(
        "--sourceurl",
        help="Source URL without trailing slash",
        type=str,
        default="https://tools.clariah.nl",
    )
    parser.add_argument(
        "--sourcetemplate",
        help="Source URL Template",
        type=str,
        default="https://tools.clariah.nl/{source-item-id}",
    )
    parser.add_argument(
        "--minrating",
        type=int,
        help="Minimal rating for tools to be propagated to the marketplace",
        default=3,
    )
    parser.add_argument("--debug", help="Debug mode", action="store_true")
    parser.add_argument(
        "--force",
        help="Force update even if entries seem up to date",
        action="store_true",
    )
    parser.add_argument(
        "--ignore", help="Ignore entries that can't be converted", action="store_true"
    )
    parser.add_argument(
        "--keywords",
        help="Add these keywords (comma separated) to all entries",
        type=str,
        default="clariah,DARIAH Resource",
    )
    parser.add_argument(
        "--reviewer",
        help="Add this actor with role reviewer to all entries",
        type=str,
        default="CLARIAH-NL",
    )
    parser.add_argument(
        "--pause",
        help="Pause this many seconds between tools (may help with rate limiting)",
        type=int,
        default=0,
    )
    ...
    api = MarketPlaceAPI(
        args.baseurl, args.username, args.password, args.token, args.debug
    )
    source = api.get_or_add_source(
        args.sourcelabel, args.sourceurl, args.sourcetemplate
    )
        ...
        for res, _, _ in g.triples((None, RDF.type, SDO.SoftwareSourceCode)):
            ...
            if args.minrating:
                ...
                if not passrating:
                    ...
                    print(
                        f"--- Tool {name} does not pass rating threshold, skipping  ---",
                        file=sys.stderr,
                    )
                    continue
            ...
            for _, _, license in g.triples((res, SDO.license, None)):
                if str(license).startswith(("http://spdx.org", "https://spdx.org")):
                    ...
                    if str(license).startswith(("http://spdx.org", "https://spdx.org")):
                        ...
                        print(
                            f"WARNING: Not an SPDX license {res}: {license} .. skipping...",
                            file=sys.stderr,
                        )
                        continue
                try:
                    licensedata = api.get_license(
                        code
                    )  # all spdx licenses are already in the marketplace database, we're not inserting new ones
                except KeyError as e:
                    if args.ignore:
                        continue
                    else:
                        print(
                            "WARNING: Unknown licence (can't map, skipping entire tool!):",
                            code,
                            file=sys.stderr,
                        )
                        raise e
            ...
            for _, _, targetproduct in g.triples((res, CODEMETA.isSourceCodeOf, None)):
                ...
                for _, _, interfacetype in g.triples((targetproduct, RDF.type, None)):
                    if str(interfacetype).startswith(NS_INVOCATION_TYPE):
                        ...
                        print(
                            "WARNING: Unknown targetProduct type (can't map):",
                            interfacetype,
                            file=sys.stderr,
                        )
            ...
            if license_found and not has_targetproduct_url:
                ...
                properties.append(
                    {"type": {"code": "terms-of-use"}, "value": "Open Source"}
                )
                properties.append(
                    {
                ...
                properties.append(
                    {
                        "type": {"code": "terms-of-use-url"},
                        "value": str(license_found["uri"]),
                    }
                )
                properties.append({"type": {"code": "terms-of-use"}, "value": "Free"})
            ...
            if not lifecycle_status:
                for _, _, devstatus in g.triples(
                    (res, CODEMETA.developmentStatus, None)
                ):
            ...
            if lifecycle_status:
                properties.append(
                    {
                        "type": {"code": "life-cycle-status"},
                        "concept": {
                            "code": str(lifecycle_status).split("/")[-1],
                            "vocabulary": {"code": "life-cycle-status"},
            ...
            if trl:
                properties.append(
                    {
                        "type": {"code": "technology-readiness-level"},
                        "concept": {
                            "code": trl.split("/")[-1],
                            "vocabulary": {"code": "technology-readiness-level"},
                            "uri": trl,
                        },
                    }
                )
                    ...
                    external_ids.append(
                        {
                            ...
                            "identifier": str(sourcerepo).replace(
                                "https://github.com/", ""
                            ),
                        }
                    )
            ...
            for _, _, o in g.triples((None, SDO.inLanguage, None)):
                if str(o).startswith("https://vocabs.acdh.oeaw.ac.at/iso6393/") or str(
                    o
                ).startswith("https://iso639-3.sil.org/code/"):
